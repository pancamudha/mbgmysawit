    ...
    <div className="w-full">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-5 mb-10">
        {displayedAnimes.map((anime, index) => {
          ...
          return (
            // PERUBAHAN: Tambahkan active:scale-[0.98] dan transition-transform
            <Link href={`/anime/${anime.slug}`} key={`${anime.slug}-${index}`} className="group cursor-pointer flex flex-col active:scale-[0.98] transition-transform duration-200">
              {/* PERUBAHAN: Tambahkan group-active:ring-2 dan group-active:-translate-y-1.5 */}
              <div className="relative aspect-[3/4] rounded-xl overflow-hidden bg-[#1A1A1C] mb-2.5 group-hover:ring-2 group-active:ring-2 ring-white/10 transition-all duration-300 group-hover:-translate-y-1.5 group-active:-translate-y-1.5">
                ...
                {/* PERUBAHAN: Tambahkan group-active:bg-black/50 */}
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/50 group-active:bg-black/50 transition-colors duration-300 z-20 flex items-center justify-center">
                  {/* PERUBAHAN: Tambahkan group-active:opacity-100 */}
                ...
                {anime.release_day && (
                  <div className="absolute bottom-2 left-2 text-[10px] font-medium text-emerald-400 uppercase z-30">
                    {anime.release_day}
                  </div>
                )}
              </div>
              ...
              <div className="flex items-start gap-1.5">
                ...
                {/* PERUBAHAN: Tambahkan group-active:text-indigo-400 */}
              ...
              <div className="flex items-center gap-2 mt-1 text-[11px] font-semibold text-[#8C8C8C] uppercase tracking-wide">
                ...
                {anime.current_episode && (
                  <span className="flex items-center gap-1">
                    ...
                    {anime.current_episode.replace(/\D/g, "")}
                  </span>
                )}
              </div>
            </Link>
