...
export default function AnimeRelated({ relations = [], seasons = [], hasTrailer = false, trailerNode }: AnimeRelatedProps) {
  ...
  // Kita gunakan deteksi di CSS className nanti. Tapi secara logical kita pisah array-nya
  ...
  // Jika tidak, biarkan CSS yang memotong jumlah yang tampil (8 di desktop, 4 di mobile)
          ...
          <div className={`grid gap-3 sm:gap-4 ${
            ...
            {(hasTrailer ? sidebarItems : fullWidthItems).map((anime, idx) => {
              ...
              if (!isViewAll) {
                 if (hasTrailer) {
                    // Sidebar mode:
                    // Max 8 di tablet (grid 2), Max 4 di desktop (grid 1), Max 4 di mobile (grid 1)
                    if (idx >= 8) displayClass = "hidden";
                    else if (idx >= 4) displayClass = "hidden sm:block lg:hidden"; 
                 } else {
                    // Full Width mode:
                    // Max 8 di desktop/tablet, Max 4 di mobile
                    if (idx >= 8) displayClass = "hidden";
