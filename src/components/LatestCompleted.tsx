...
export default function LatestCompleted({ animes = [] }: { animes: AnimeItem[] }) {
  ...
  const { getTitle } = useAnimeTitle(); // Mengambil hook bahasa
    ...
    <div className="w-full">
      ...
      <div className={`flex flex-col gap-2.5 transition-all duration-300 ${
        ...
        {displayedList.map((anime) => {
          ...
          const releaseYear = anime.tvInfo?.releaseDate ? anime.tvInfo.releaseDate.split(', ').pop() : new Date().getFullYear();
          const displayTitle = getTitle(anime.title, anime.japanese_title);

          return (
            <Link href={`/anime/${anime.id}`} key={anime.id} className="group relative flex items-center gap-3.5 p-2 rounded-xl overflow-hidden bg-[#141414] border border-white/5 hover:border-white/10 active:border-white/10 transition-all duration-300 shrink-0">
