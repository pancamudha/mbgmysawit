...
export default function AZFilterBar() {
  ...
  const currentLetter = params.letter ? params.letter[0].toLowerCase() : "all";
              ...
              <Link
                ...
                className={`flex items-center justify-center h-9 sm:h-10 ${dynamicPadding} rounded-[10px] text-[13px] sm:text-sm font-medium transition-all min-w-[36px] sm:min-w-[40px] ${
                  isActive
                    ...
                    : "bg-[#0F0F0F] text-[#8C8C8C] border border-[#1C1C1F] hover:border-[#2A2A2E] hover:text-white hover:bg-[#161616]" // Style default/hover
                }`}
              >
                {item.label}
              </Link>
