...
export default function Footer() {
  return (
    <footer className="w-full flex flex-col pt-2">
      
      {/* === Bagian Atas: Logo Bebas === */}
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 w-full flex justify-center sm:justify-start">
        <Link href="/" className="flex items-end w-fit hover:opacity-80 transition-opacity">
          <div className="flex items-end relative">
            
            {/* Logo Custom Animaple */}
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img 
              src="/logo.webp" 
              alt="Animaple Logo" 
              className="h-[46px] sm:h-[54px] w-auto object-contain drop-shadow-sm relative z-10 -mb-[1px]" 
            />
            
            {/* Teks Brand */}
            <div className="flex flex-col justify-end ml-2 pb-1.5 sm:pb-2.5 leading-none">
              <span className="text-[24px] sm:text-[28px] font-black text-white tracking-tight">ANIMAPLE</span>
            </div>
      ...
      {/* === Bagian Bawah: Strip Full-Width (Teks & Link) === */}
      <div className="w-full bg-[#050505] border-t border-white/5 relative z-0">
        
        {/* Container pembungkus A-Z list dan informasi Footer */}
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-7 flex flex-col gap-8">
          
          {/* === A-Z List Section === */}
          <div className="w-full flex flex-col items-center md:items-start gap-3">
            <div className="flex items-baseline justify-center md:justify-start gap-3 text-center md:text-left">
              <h2 className="text-[15px] sm:text-[16px] font-bold text-white tracking-wide">A-Z LIST</h2>
          ...
          {/* MAGISNYA DI SINI: flex-col-reverse membuat Kanan (Sosmed) naik ke atas Kiri (Teks) di Mobile */}
          <div className="flex flex-col-reverse md:flex-row md:items-center justify-between gap-6">
            
            {/* Kiri: Teks Sanggahan & Hak Cipta */}
            <div className="flex flex-col gap-1 sm:gap-1.5 items-center text-center md:items-start md:text-left">
              <p className="text-[#8C8C8C] text-[12px] sm:text-[13px] font-medium max-w-[500px] md:max-w-full">
                Animaple does not store any files on our server, we only link to media hosted on 3rd-party services.
              </p>
              <p className="text-[#8C8C8C] text-[12px] sm:text-[13px] font-medium">
                © {new Date().getFullYear()} Animaple. All rights reserved.
              </p>
            </div>

            {/* Kanan: Navigasi Horizontal & Ikon Raw */}
            <div className="flex flex-col-reverse md:flex-row flex-wrap items-center gap-5 sm:gap-8 justify-center md:justify-end w-full md:w-auto">
              
              {/* Tautan */}
              <div className="flex flex-wrap justify-center items-center gap-4 sm:gap-5 text-[12px] sm:text-[13px] font-semibold text-[#8C8C8C]">
                <Link href="/terms" className="hover:text-white transition-colors">Terms of Service</Link>
                <Link href="/dmca" className="hover:text-white transition-colors">DMCA</Link>
                <Link href="/donate" className="hover:text-white transition-colors">Donate &lt;3</Link>
              </div>
              
              {/* Ikon Sosial Media (Polosan) */}
